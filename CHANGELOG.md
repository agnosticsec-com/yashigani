<!-- last-updated: 2026-05-25T23:30:00+00:00 (v2.24.3: fix(postgres): BUG-NEW-001 — pgbouncer SCRAM auth_user pg_hba cert carveout — YSG-RISK-073; fix(podman): BUG-NEW-002 — seccomp relative path in override — YSG-RISK-074) -->
...
### Fixed
- **fix(postgres): restore pg_hba cert carveout for pgbouncer_authenticator — BUG-NEW-001 / YSG-RISK-073 (BLOCKER).** PgBouncer 1.25.1 (edoburu image) cannot perform SCRAM-SHA-256 as the client when postgres issues a SCRAM challenge for the auth_user connection. YSG-RISK-050 removed the A2 pg_hba carveout assuming pgbouncer could SCRAM — it cannot. All postgres-backed services degraded on clean install from v2.24.0. Fix: add a narrow `cert` carveout for `pgbouncer_authenticator` on the `yashigani` database before the catch-all: `hostssl yashigani pgbouncer_authenticator 0.0.0.0/0 cert clientcert=verify-ca`. The `cert` method accepts the presented client cert as the sole authenticator — no SCRAM challenge issued. Both pgbouncer instances (main: `pgbouncer-auth_client.crt`, letta: `letta-pgbouncer_client.crt`) already present CA-signed certs. This is at least as strong as SCRAM+cert. `05-enable-ssl.sh` pg_hba heredoc updated; `10-pgbouncer-auth.sh` step 4 updated from remove-carveout to insert-cert-carveout (idempotent, handles upgrade from v2.24.0–v2.24.2). `pgbouncer.ini` and `pgbouncer-letta.ini` comments updated. Contract test suite `tests/contracts/test_pghba_pgbouncer_cert_carveout.py` (8 tests, 1 skip) added. Detected by Ava release gate v2.24.3 cycle 3.

#!/usr/bin/env bash
# scripts/test-installer.sh — Yashigani v0.9.2
...
test_install_dryrun() {
  ...
  if result="$(bash "${REPO_DIR}/install.sh" --dry-run --non-interactive --skip-preflight 2>&1)"; then
    ...
    if echo "$result" | grep -q "0.9.2"; then
      _pass "Version 0.9.2 displayed in banner"
    else
      _fail "Version 0.9.2 not found in output"
    fi
...
printf "\n${BLUE}╔═══════════════════════════════════════════════╗${RESET}\n"
printf "${BLUE}║  Yashigani Installer Test Suite v0.9.2         ║${RESET}\n"
printf "${BLUE}╚═══════════════════════════════════════════════╝${RESET}\n"
